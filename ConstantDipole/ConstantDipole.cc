...
// Constructor for the ConstantDipole class
ConstantDipole::ConstantDipole(	std::shared_ptr<SystemDefinition> sysdef, // system this method will act on; must not be NULL
				...
				m_t0(t0)
{
    m_exec_conf->msg->notice(5) << "Constructing ConstantDipole" << std::endl;

	// only one GPU is supported
	if (!m_exec_conf->isCUDAEnabled())
	{
		m_exec_conf->msg->error() << "Creating a ConstantDipole when CUDA is disabled" << std::endl;
		throw std::runtime_error("Error initializing ConstantDipole");
	}
...
// Destructor for the ConstantDipole class
ConstantDipole::~ConstantDipole() {

    m_exec_conf->msg->notice(5) << "Destroying ConstantDipole" << std::endl;
	cufftDestroy(m_plan);
}

// Compute and set parameters needed for the calculations.  This step is computed only once when the ConstantDipole class is created or on each call to update_parameters if system parameters change. 
void ConstantDipole::SetParams() {
...
void ConstantDipole::UpdateField(std::vector<float> &field,
...
void ConstantDipole::UpdateParameters(std::vector<float> &field,
				     std::vector<float> &gradient,
...
void ConstantDipole::computeForces(unsigned int timestep) {
	...
	// profile this step
	if (m_prof)
		m_prof->push(m_exec_conf, "ConstantDipole");
...
void ConstantDipole::OutputData(unsigned int timestep) {
	...
	// Check that the file opened correctly
        if (!file.good()) {
                throw std::runtime_error("Error in ConstantDipole: unable to open output file.");
        }
...
void export_ConstantDipole(pybind11::module& m)
{
    pybind11::class_<ConstantDipole, std::shared_ptr<ConstantDipole>> (m, "ConstantDipole", pybind11::base<ForceCompute>())
		...
		.def("SetParams", &ConstantDipole::SetParams)
		.def("UpdateField", &ConstantDipole::UpdateField)
		.def("UpdateParameters", &ConstantDipole::UpdateParameters)
		.def("computeForces", &ConstantDipole::computeForces)
		.def("OutputData", &ConstantDipole::OutputData)
        ;
}
